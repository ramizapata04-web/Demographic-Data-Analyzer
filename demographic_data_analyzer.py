...
def calculate_demographic_data(print_data=True):
    ...
    highest_earning_country = (df.groupby('native-country')['salary'].apply(lambda x: (x == '>50K').sum() / len(x) * 100).round(1).idxmax())
    highest_earning_country_percentage = (df.groupby('native-country')['salary'].apply(lambda x: (x == '>50K').sum() / len(x) * 100).round(1).max())
